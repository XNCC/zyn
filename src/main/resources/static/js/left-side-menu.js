$(function(){
    ...
    $('.lsm-sidebar a').on('click',function(){
        ...
        if (!$('.left-side-menu1').hasClass('lsm-mini')) {
    ...
    //lsm-mini
    $('.lsm-mini-btn1 svg').on('click',function(){
        if ($('.lsm-mini-btn1 input[type="checkbox"]').prop("checked")) {
            $('.lsm-sidebar-item.lsm-sidebar-show').removeClass('lsm-sidebar-show');
            $('.lsm-container ul').removeAttr('style');
            $('.left-side-menu1').addClass('lsm-mini');
            $('.left-side-menu1').stop().animate({width : 60},200);
        }else{
            $('.left-side-menu1').removeClass('lsm-mini');
            $('.lsm-container ul ul').css("display", "none");
            $('.left-side-menu1').stop().animate({width: 240},200);
        }
